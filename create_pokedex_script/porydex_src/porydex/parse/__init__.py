...
def process_ternary(expr: TernaryOp) -> typing.Any:
    # Evaluate the condition using process_binary so nested ops (||, &&) are handled correctly.
    cond = expr.cond
    if isinstance(cond, BinaryOp):
        cond_result = process_binary(cond)
    elif isinstance(cond, ID):
        raise ValueError('cannot process bare ID condition in ternary')
    else:
        raise ValueError(f'unsupported ternary condition type: {type(cond)}')
    if cond_result:
        return expr.iftrue
    else:
        return expr.iffalse
...
def extract_u8_str(expr) -> str:
    ...
    # Some entries (e.g. ITEM_NONE) use a variable reference rather than an inline string.
...
def extract_int(expr) -> int:
    if isinstance(expr, TernaryOp):
        # Recurse so the result branch (which may itself be an ID or BinaryOp) is handled correctly
        return extract_int(process_ternary(expr))
    ...
    # Enum constants stay as ID nodes after preprocessing (unlike #define macros).
    # Fall back to a lookup table of known expansion enum values.
    if isinstance(expr, ID):
        if expr.name in EXPANSION_ENUMS:
            return EXPANSION_ENUMS[expr.name]
        raise ValueError(f'unknown expansion enum constant: {expr.name}')
